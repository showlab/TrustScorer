...
from assistgui.model.command.additional_utills import highlight_img_case3, highlight_img_case12, img2base64


# read and load JSON file of metadata
...
''')
    ...
    def __call__(self, pre_img, cur_img, current_gui, history): # path, gui_meta, previous_code, current_code):
        ...
        input_pre_img, input_lat_img = self.highlight_img(pre_img, cur_img, previous_gui, current_gui, code)
        success_flag, resume_flag, message, next_step = self.action_evaluation_GPT4V(task, previous_code, code,
                                                                                     input_pre_img, input_lat_img)
        return success_flag, resume_flag, message, next_step

    def action_evaluation_GPT4V(self, task, pre_code, cur_code, input_pre_img, input_lat_img):
        task = task.split(":")[1]
        ...
        text = f'''An AI is operating the keyboard and mouse, aiding a user with tasks. 
...
In the following, 2 images which shows the states of the program will be provided, which only shows the main operation areas.'''
        ...
        prompt = [
            ...
            {
                "type": "text",
                "text": "The image before Action:"
            },

            {
                "type": "image_url",
                "image_url": {
                    "url": f"data:image/jpeg;base64,{input_pre_img}",
                }
            },
            {
                "type": "text",
                "text": "The image after Action:"
            },

            {
                "type": "image_url",
                "image_url": {
                    "url": f"data:image/jpeg;base64,{input_lat_img}",
                }
            }
        ]
    ...
    def highlight_img(self, image1, image2, previous_gui, current_gui, code=""):
        ...
        if len(panel_diff_copy['added']) > 0 and len(panel_diff_copy['removed']) == 0:
            print("Case 1:")
            base64_image_pre, base64_image_lat = highlight_img_case12(image1, image2)
        ...
        # Case 3: Manipulate one panel
            ...
            print("Case 3:")
            base64_image_pre, base64_image_lat = highlight_img_case3(image1, image2, previous_gui, current_gui,
                                                                     self.extract_last_position(code))

        # Other cases
        else:
            base64_image_pre, base64_image_lat = img2base64(image1, image2)
